...
def q2(n):
    ...
    '''
    t1 = np.array([range(n)] * n).T
    t2 = np.array([range(n)] * n)
    return t1 + t2
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        """
        # 你的实现代码
        cur_layout = [[' '] * self.width for x in range(self.height)]
        for x, y in self.walls:
            cur_layout[x][y] = '%'
        for x, y in self.foods:
            cur_layout[x][y] = '.'
        cur_layout[self.pacman_pos[0]][self.pacman_pos[1]] = 'P'
        return '\n'.join([''.join(i) for i in cur_layout])
