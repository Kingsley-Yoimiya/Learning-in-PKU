...
def buildtrees(X, Y):
    ...
    for i in range(num_tree):
        idx = np.random.choice(X.shape[0], int(X.shape[0] * ratio_data), replace=False)
        idy = np.random.choice(X.shape[1], int(X.shape[1] * ratio_feat), replace=False)
        trees.append(buildTree(X[idx,], Y[idx], unused = list(idy), depth=hyperparams["depth"], purity_bound=hyperparams["purity_bound"], gainfunc=hyperparams["gainfunc"]))
    return trees
