...
class YTModel:
    def __init__(self) -> None:
        ...
        with open(YT.save_path_forest, "rb") as f:
            self.roots = pickle.load(f)

    def __call__(self, figure):
        self.graph.flush()
        pred = self.graph.forward(figure, removelossnode=True)[-1]
        #print("PREDICT:", np.argmax(pred, axis=-1), pred.shape, pred, pred.sum())
        t = np.argmax(pred, axis=-1)
        if pred[0][t] >= 0.15:
            return t
        else:
            return YT.answerRandomForest.infertrees(self.roots, YT.discretize(figure.flatten()))
