...
hyperparams = {
    "depth": 2, 
    "purity_bound": 1e-1, 
    "gainfunc": "negginiDA"  # gain giniD gainratio
    }
...
def entropy(Y: np.ndarray):
    ...
    """
    # YOUR CODE HERE
    unique_ele, cnt = np.unique(Y, return_counts=True)
    return -np.average(np.log(cnt / Y.shape[0]) * cnt / Y.shape[0])
...
def gain(X: np.ndarray, Y: np.ndarray, idx: int):
    ...
    ret = 0
    for x in range(len(ufeat)):
        ret += featcnt[x] / feat.shape[0] * entropy(Y[feat == ufeat[x]]) 
    # print(entropy(Y) - ret)
    return entropy(Y) - ret
    # YOUR CODE HERE
...
def buildTree(X: np.ndarray, Y: np.ndarray, unused: List[int], depth: int, purity_bound: float, gainfunc: Callable, prefixstr=""):
    ...
    gains = [negginiDA(X, Y, i) for i in unused]
    # print(gains)
    idx = np.argmax(gains)
